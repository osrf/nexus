...
#ifndef NEXUS_COMMON__NODE_UTILS_HPP_
#define NEXUS_COMMON__NODE_UTILS_HPP_
...
/// \brief Replace invalid characters in a potential node name
///
...
NEXUS_COMMON_EXPORT std::string sanitize_node_name(
  const std::string& potential_node_name);

/// \brief Concatenate two namespaces to produce an absolute namespace
///
...
NEXUS_COMMON_EXPORT std::string add_namespaces(
  const std::string& top_ns,
  const std::string& sub_ns = "");

/// \brief Add some random characters to a node name to ensure it is unique in the system
///
...
NEXUS_COMMON_EXPORT std::string generate_internal_node_name(
  const std::string& prefix = "");

/// \brief Creates a node with a name as generated by generate_internal_node_name
///
...
NEXUS_COMMON_EXPORT rclcpp::Node::SharedPtr generate_internal_node(
  const std::string& prefix = "");
...
NEXUS_COMMON_EXPORT std::string time_to_string(size_t len);

NEXUS_COMMON_EXPORT
rclcpp::NodeOptions
get_node_options_default(
  ...
  const std::string& plugin_name)
{
  ...
  try
  {
    ...
    {
      RCLCPP_FATAL(
        node->get_logger(), "Can not get 'plugin' param value for %s",
        plugin_name.c_str());
      exit(-1);
    }
  }
  ...
  {
    RCLCPP_FATAL(
      node->get_logger(), "'plugin' param not defined for %s",
      plugin_name.c_str());
    exit(-1);
  }
...
/**
 * Creates an executor and binds it to the same context as the node.
 */
template<typename ExecutorT = rclcpp::executors::SingleThreadedExecutor,
  typename NodePtrT>
ExecutorT create_executor(NodePtrT node)
{
  auto context = node->get_node_base_interface()->get_context();
  rclcpp::ExecutorOptions opts;
  opts.context = context;
  return ExecutorT(opts);
}

}  // namespace nexus::common

#endif  // NEXUS_COMMON__NODE_UTILS_HPP_
