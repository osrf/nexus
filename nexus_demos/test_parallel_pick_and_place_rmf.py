...
class ParallelPickAndPlaceRmfTest(NexusTestCase):
    ...
    async def test_parallel_pick_and_place_wo(self):
        ...
        # Forth goal
        goal_msg.order.work_order_id = "4"
        forth_feedbacks: list[ExecuteWorkOrder.Feedback] = []
        forth_fb_fut = Future()

        def on_forth_fb(msg):
            forth_feedbacks.append(msg.feedback)
            if len(forth_feedbacks) >= 5:
                forth_fb_fut.set_result(None)

        forth_goal_handle = cast(
            ClientGoalHandle, await self.action_client.send_goal_async(goal_msg, on_forth_fb)
        )
        self.assertTrue(forth_goal_handle.accepted)
        ...
        self.assertEqual(results.status, GoalStatus.STATUS_SUCCEEDED)
        results = await forth_goal_handle.get_result_async()
        self.assertEqual(results.status, GoalStatus.STATUS_SUCCEEDED)
