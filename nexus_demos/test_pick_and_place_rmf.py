...
class PickAndPlaceTest(NexusTestCase):
    ...
    async def test_pick_and_place_wo(self):
        ...
        goal_msg.order.id = 'test_pick_and_place_rmf'
        ...
        for msg in feedbacks:
            ...
            self.assertEqual(state.task_id, "test_pick_and_place_rmf/place_on_conveyor")
            state: TaskState = msg.task_states[2]  # type: ignore
            self.assertEqual(state.workcell_id, "workcell_2")
            self.assertEqual(state.task_id, "test_pick_and_place_rmf/pick_from_conveyor")
