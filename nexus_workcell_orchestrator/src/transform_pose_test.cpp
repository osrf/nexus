...
TEST_CASE("ApplyPoseOffset") {
  common::test::RosFixture<rclcpp_lifecycle::LifecycleNode> fixture;
  BT::BehaviorTreeFactory factory;
  factory.registerBuilder<ApplyPoseOffset>("ApplyPoseOffset",
    [&](const std::string& name, const BT::NodeConfiguration& config)
    {
      return std::make_unique<ApplyPoseOffset>(name, config, *fixture.node);
    });
  ...
  SECTION("local transform") {
    auto bt = factory.createTreeFromText(
      ...
      <root>
        <BehaviorTree>
          <ApplyPoseOffset base_pose="{base_pose}" transform="{transform}" local="true" result="{result}" />
        </BehaviorTree>
      </root>
    )");
  ...
  SECTION("parent frame transform") {
    auto bt = factory.createTreeFromText(
      ...
      <root>
        <BehaviorTree>
          <ApplyPoseOffset base_pose="{base_pose}" transform="{transform}" local="false" result="{result}" />
        </BehaviorTree>
      </root>
    )");
...
TEST_CASE("GetPoseOffset") {
  ...
  BT::BehaviorTreeFactory factory;
  factory.registerBuilder<GetPoseOffset>("GetPoseOffset",
    [&](const std::string& name, const BT::NodeConfiguration& config)
    {
      return std::make_unique<GetPoseOffset>(name, config, *fixture.node,
      tf2_buffer);
    });
  ...
  auto bt = factory.createTreeFromText(
    R"(
      <root>
        <BehaviorTree>
          <GetPoseOffset base_pose="{base_pose}" target_pose="{target_pose}" time="{lookup_time}" result="{result}" />
        </BehaviorTree>
      </root>
    )");
