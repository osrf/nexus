...
{
  this->manage_service_name_ = "/" + name + std::string("/manage_nodes");
  this->active_service_name_ = "/" + name + std::string("/is_active");
  ...
  manager_client_ =
    std::make_shared<common::SyncServiceClient<ManageLifecycleNodes>>(
    node_, manage_service_name_);
  is_active_client_ =
    std::make_shared<common::SyncServiceClient<std_srvs::srv::Trigger>>(
    node_, active_service_name_);
}
...
LifecycleManagerClient::is_active(const std::chrono::nanoseconds timeout)
{
  ...
  try
  {
    response = is_active_client_->send_request(request, timeout);
  }
  ...
  const std::chrono::nanoseconds timeout)
{
  ...
  RCLCPP_DEBUG(
    node_->get_logger(), "Waiting for the %s service...",
    this->manage_service_name_.c_str());
  ...
  try
  {
    auto future_result = manager_client_->send_request(request, timeout);
